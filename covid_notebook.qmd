...
plot(residential_ts,  ylab = "Pourcentage de déplacement vers les lieux de résidences", title = "Mobilité vers les lieux de résidences jour par jour")
# Aggrégation par moyenne des valeurs par semaine
residential_ts1 <- aggregate(residential_ts, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(residential_ts1, ylab = "Pourcentage de déplacement vers les lieux de résidences", title = "Mobilité vers les lieux de résidences semaine par semaine en omettant les weekends")
```

Le premier graphique montre la variable "residential" qui est donc le mobilité à partir du lieu de résidence. Cela nous permet donc de déduire le temps passé au domicile ou en extérieur. Tout d'abord, début 2020, on observe un grand bond en début de série (la mobilité résidentielle atteint des valeurs de 20 à 35) et cette date correspond au premier confinement. De mi à fin 2020, après le premier pic, les valeurs redescendent vers 10-15 mais restent tout de même élevées. On observe malgré tout quelques pics qui pourraient correspondre aux reconfinements d'automne 2020. Ensuite en 2021, la mobilité résidentielle décroit. Et pour terminer en 2022, on observe des valeurs plus basses avec quelques oscillations et pics sporadiques.

Le second graphique montre la variable 'residential" tout comme le premier graphique mais cette fois-ci, nous avons enlevé les weekends et fait une "moyenne" de la mobilité par semaine. Cela permet d'éliminer du bruit journalier et donc de donner une tendance plus nette grâce au retrait des weekends. Tout comme dans la série originale, un pic massif est observé au début de la série ce qui correspondrait au premier confinement. Mi mai 2020, le pic massif redescend ceci est du au déconfinement. De la fin de 2020 à 2021, on observe des cycles réguliers et une baisse. Les oscillations ont peut être été causé par des reconfinements et des couvre feu, télétravail... Pour terminer, en 2022, les valeurs commence à redescendre avec des pics moins élevés.

Etant donné que le second graphique a moins de bruit journalier, nous allons utilisé celui là pour les différentes analyses que nous allons faire sur notre analyse.
