...
# stationnariser avant avec tsd() et boxplot pq pas
plot(extract(decdiff(residential_ts, lag = 6, order = 1), components = 1))
spectrum(residential_ts, spans = c(3,7))
cpgram(residential_ts)
# stat slide ?? statistiques glissantes ???
```
...
Le périodigramme lissé qui a été tracé montre 3 zones qui peuvent être considérées comme significatives. ???
Cependant, le périodigramme cumulé ne montre pas de périodicité significative. ????
...
La tendance générale est significative à l'augmentation de la série. ????
...
Pour continuer l'analyse de façon plus poussée, il faut choisir une seule série cohérente. La série jour par jour va être d'office éliminée car elle contient énormément de bruit journalier ce qui la rend moins lisible et pourrait causer des problèmes au niveau de l'autocorrélation. De plus, cette série a été analysée comme série initiale. Il peut être intéressant de prendre les deux série semaine par semaine (avec et sans les weekends) pour voir s'il y a ou non une différence entre les deux. On pourrait s'attendre à peu de différence car le covid a principalement influencé le travail en semaine comparée aux loisirs en weekends. 
...
avant <- window(residential_ts1, start = c(2020, 11), end = c(2021, 23))
plot(avant, xlab = "Temps", ylab = "Variation dans la présence au travail (%)", main = "Période de confinement: Jours de la semaine agrégés")
abline(h = 0, col = "red", lty = 2)

acf(avant, main = "ACF de la série ST confinement")

set.seed(123)
avant_tg <- trend.test(avant, R = 999)
plot(avant_tg, main = "Tendance générale de la série ST confinement")

avant_tl <- local.trend(avant)
plot(avant_tl, main = "Tendances locales - Période de confinement")
...
après <- window(residential_ts1, start = c(2021, 23), end = c(2022, 41))
plot(après, xlab = "Temps", ylab = "Variation dans la présence au travail (%)", main = "Période post confinement: Jours de la semaine agrégés")
abline(h = 0, col = "red", lty = 2)

acf(après, main = "ACF de la série ST post confinement")

set.seed(321)
après_tg <- trend.test(après, R = 999)
plot(après_tg, main = "Tendance générale de la série ST Post confinement")

après_tl <- local.trend(après)
plot(après_tl, main = "Tendances locales - Période post-confinement")
```
balablab
